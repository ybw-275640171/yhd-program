...
let oUser = $(".username")[0];
let oUserbtm = $(".usernamebottom")[0];
let oPhone = $(".userphone")[0];
let oPhonebtm =$(".userphonebottom")[0];
let oPass = $(".userpass")[0];
let oPassbtm =$(".userpassbottom")[0];
let oPass1 = $(".userpass1")[0];
let oPassbtm1 =$(".userpassbottom1")[0];
var isRight = [0,0,0,0]
...
oUser.onblur = function(){
    ...
    xhr.onreadystatechange = ()=>{
        if(xhr.readyState==4  && xhr.status==200){
            // 5、接收响应
            let str =  xhr.responseText;  //1或者0；1：表示该用户名已经存在；0：表示该用户名不存在
            console.log(str);
            if(str == 1){
                isRight[0]=0;
...
let oForm = document.getElementById("f");
console.log(oForm);
